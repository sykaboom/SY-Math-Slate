# Task 383: Theme JSON IO Preview Rollback
...
## Goal (Base Required)
- What to change:
  - Theme JSON IO Preview Rollback를 P5 단계에서 실행 가능한 단위로 구현한다.
...
- v10/src/features/chrome/theming-ui/io, v10/src/core/config/themeTokens.ts, v10/src/features/chrome/theming-ui
...
- [ ] AC-1: JSON import/export 스키마 검증
- [ ] AC-2: preview->commit->rollback 플로우 보장
- [ ] AC-3: invalid payload 안전거부
...
1) Step:
   - Command / click path:
     - import/export roundtrip + malformed JSON 테스트
   - Expected result:
     - AC-1, AC-2 만족
   - Covers: AC-1, AC-2
...
- Risks:
  - 잘못된 payload 적용
- Roll-back:
  - JSON apply 경로 disable 후 revert
