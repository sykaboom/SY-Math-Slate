# Task 365: Edge Snap UX and Canvas Safe Insets
...
## Goal (Base Required)
- What to change:
  - Implement Samsung Notes-style edge snap docking while preserving canvas non-occlusion in target tablet viewports.
- What must NOT change:
  - Do not add unrelated toolbar actions or new panels.
...
- Files shared with other PENDING tasks:
  - AppLayout panelAdapters shared
...
- [ ] AC-1: Drag release snaps toolbar to nearest valid edge with deterministic thresholds.
- [ ] AC-2: Canvas insets adapt so slate drawing area is never occluded by docked toolbar.
- [ ] AC-3: Target tablet viewports pass redline checks.
...
1) Step:
   - Command or click path:
     - Validate drag snap and insets on all four target viewports.
   - Expected result:
...
- Risks:
  - Snap thresholds can feel inconsistent across devices if not normalized.
- Roll-back:
  - Rollback to previous docking policy and inset resolver.
