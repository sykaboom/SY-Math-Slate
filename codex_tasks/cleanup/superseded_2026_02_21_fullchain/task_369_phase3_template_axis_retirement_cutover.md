# Task 369: Template Axis Retirement Cutover
...
## Goal (Base Required)
- What to change:
  - Retire dual-axis template mod ownership and complete single-axis ModPackage to Mod runtime authority.
- What must NOT change:
  - Do not change visible UX semantics except where legacy duplication is removed.
...
- Files shared with other PENDING tasks:
  - registry store toolbar mapping shared
...
- [ ] AC-1: Legacy template registry path is removed or strictly bridged with deprecation path.
- [ ] AC-2: Toolbar package activation mapping uses package policy selectors only.
- [ ] AC-3: No runtime reference remains to retired dual-axis ownership path.
...
1) Step:
   - Command or click path:
     - Run rg search for templatePackRegistry and legacy dual-axis markers.
   - Expected result:
...
- Risks:
  - Cutover can break old package identifiers if mapping table is incomplete.
- Roll-back:
  - Restore adapter bridge while keeping new contract intact.
