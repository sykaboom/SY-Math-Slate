# Task 362: Authority Sync and Roadmap Lock
...
## Goal (Base Required)
- What to change:
  - Align AGENTS, roadmap, context, and task references so one active execution source exists.
- What must NOT change:
  - Do not change product behavior or user-facing UI.
...
- PROJECT_ROADMAP.md, PROJECT_CONTEXT.md, AGENTS.md references, task_362 spec

Out of scope:
- Any runtime logic changes are excluded.
...
- [ ] AC-1: Roadmap clearly labels active phases and archives superseded plans.
- [ ] AC-2: Authority order references are consistent across governance docs.
- [ ] AC-3: No conflicting active plan statements remain.
...
1) Step:
   - Command or click path:
     - Search conflicting markers in governance documents.
   - Expected result:
...
2) Step:
   - Command or click path:
     - Run rg for ACTIVE SSOT Historical superseded markers.
   - Expected result:
...
- Risks:
  - Documentation drift if future updates skip this lock policy.
- Roll-back:
  - Revert only governance docs touched by this task.
