# Task 366: Input Gateway Unification
...
## Goal (Base Required)
- What to change:
  - Centralize pointer key wheel ingress into one normalized gateway before mod routing.
- What must NOT change:
  - Do not alter mod behavior semantics beyond routing path consolidation.
...
- Files shared with other PENDING tasks:
  - mod host runtime bridge shared
...
- [ ] AC-1: All primary input sources pass through one normalization gateway.
- [ ] AC-2: Gateway forwards to mod routing path with explicit handled pass contract.
- [ ] AC-3: No duplicate direct input paths remain in target scope.
...
1) Step:
   - Command or click path:
     - Inspect call graph with rg routePointer routeKey routeWheel.
   - Expected result:
...
- Risks:
  - Missed ingress paths can create silent behavior divergence.
- Roll-back:
  - Restore previous direct routing branch and disable gateway by feature flag if needed.
