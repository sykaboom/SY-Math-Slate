# Task 368: ModPackage Contract v1
...
## Goal (Base Required)
- What to change:
  - Define and lock ModPackage contract as the operational unit over single mods including capabilities and activation policy schema.
- What must NOT change:
  - Do not remove existing working packages without adapter migration path.
...
- v10/src/core/mod/package, v10/src/mod/packages, v10/src/core/mod/contracts

Out of scope:
- UI host rendering changes are excluded.
...
- Files shared with other PENDING tasks:
  - mod package runtime files shared
...
- [ ] AC-1: Package manifest schema validates pack id version mod list default mod capabilities policy.
- [ ] AC-2: Registry loads package definitions deterministically with explicit errors.
- [ ] AC-3: Contract docs and examples are updated for operators.
...
1) Step:
   - Command or click path:
     - Run schema validation and package load smoke tests.
   - Expected result:
...
- Risks:
  - Schema tightening may reject malformed package data previously tolerated.
- Roll-back:
  - Keep adapter fallback for one release and log deprecations.
