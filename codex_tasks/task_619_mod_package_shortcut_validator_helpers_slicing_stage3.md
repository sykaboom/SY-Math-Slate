# Task 619: mod-package shortcut validator helpers slicing (stage3)
...
## Goal (Base Required)
- What to change:
  - `shortcutRules/validators/entry/helpers.ts`를 facade + helper 디렉터리로 분해한다.
- What must NOT change:
  - shortcut validator entry semantics 변경 금지.
...
- `codex_tasks/task_619_mod_package_shortcut_validator_helpers_slicing_stage3.md`
...
- Boundary rules:
  - shortcut validator lane 내부 분해 only
- Compatibility:
  - `validateShortcutRuleEntryShape`, `buildShortcutRule` export 유지
...
1) Step:
   - Command / click path:
     - `wc -l v10/src/core/runtime/modding/package/guards/resourcePolicy/shortcutRules/validators/entry/helpers.ts`
   - Expected result:
     - <= 40
   - Covers: AC-1
...
- shortcut validator helpers가 `shape.ts` + `build.ts`로 분리됨.
