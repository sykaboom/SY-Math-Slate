# Task 363: Boundary Guardrails Baseline
...
## Goal (Base Required)
- What to change:
  - Boundary Guardrails Baseline를 P0 단계에서 실행 가능한 단위로 구현한다.
...
- scripts/check_layer_rules.sh, scripts/check_mod_contract.sh, v10/eslint.config.mjs
...
- [ ] AC-1: 모듈 경계 위반이 자동 검출됨
- [ ] AC-2: lint/스크립트 실패 시 명확한 오류가 출력됨
- [ ] AC-3: 기존 정상 경로는 계속 통과
...
- Risks:
  - 과도한 규칙으로 정상 import 차단
- Roll-back:
  - 규칙 변경분만 롤백하고 허용 리스트 재조정
