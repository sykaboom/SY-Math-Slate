# Task 371: Shortcut Menu Toolbar Command Tests
...
## Goal (Base Required)
- What to change:
  - Shortcut Menu Toolbar Command Tests를 P2 단계에서 실행 가능한 단위로 구현한다.
...
- [ ] AC-1: toolbar/menu/shortcut이 동일 command 경로 사용
- [ ] AC-2: 중복 mutation 경로 없음
- [ ] AC-3: 회귀 매트릭스가 문서화됨
...
1) Step:
   - Command / click path:
     - toolbar/menu/shortcut 동일 동작 수동 점검
   - Expected result:
     - AC-1, AC-2 만족
   - Covers: AC-1, AC-2
...
2) Step:
   - Command / click path:
     - bash scripts/check_mod_contract.sh && cd v10 && npm run lint
   - Expected result:
     - AC-3 만족 및 회귀 없음
   - Covers: AC-3
